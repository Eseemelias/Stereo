	...
	for i in range(5):
		for j in range(5):
			if sys.argv[4] == 'BM' or sys.argv[4] == 'bm' or sys.argv[4] == 'Bm':
				stereo = cv2.StereoBM_create(numDisparities=int(sys.argv[5]) + 16*i, blockSize=int(sys.argv[6])+10*j)
			elif sys.argv[4] == 'SGBM' or sys.argv[4] == 'sgbm' or sys.argv[4] == 'Sgbm':
				stereo = cv2.StereoSGBM_create(minDisparity = int(sys.argv[7]), numDisparities=int(sys.argv[5])+ 16*i, blockSize=int(sys.argv[6])+2*j);
			else:
			...
			plt.subplot(5,6,6*(i) + (j+1)),plt.imshow(disparity,'gray')
			plt.title('N°D: ' + str(int(sys.argv[5]) + 16*i) + ' BS: ' + str(int(sys.argv[6])+10*j))
...
if len(sys.argv) > 1:
	...
	print('G. Mínima Dispariad\n\tParámetro para el análisis de imágenes stereo, sólo en caso de usar SGBM\n\tEntero\n')
